...
class MoodScaleViewerPage extends StatelessWidget {
  ...
  List<Map<String, dynamic>> get _dummyData => [
    {"date": DateTime.now().subtract(const Duration(days: 6)), "score": 4, "emoji": "😀"},
    {"date": DateTime.now().subtract(const Duration(days: 5)), "score": 2, "emoji": "😞"},
    {"date": DateTime.now().subtract(const Duration(days: 4)), "score": 5, "emoji": "😊"},
    {"date": DateTime.now().subtract(const Duration(days: 3)), "score": 3, "emoji": "😐"},
    {"date": DateTime.now().subtract(const Duration(days: 2)), "score": 1, "emoji": "😡"},
    {"date": DateTime.now().subtract(const Duration(days: 1)), "score": 4, "emoji": "😀"},
      ...
      body: Container(
        ...
        child: ListView(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    _AnimatedMoodBarChart(data: data, maxScore: maxScore), // <--- fixed
                  ],
                ),
              ),
            ),
            ...
            Text(
              "History",
              style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown[600], fontSize: 17),
            ),
...
class _MoodSummaryCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final double avg = data.isEmpty
        ? 0
        : data.map<double>((e) => e['score']!.toDouble()).reduce((a, b) => a + b) / data.length;
    final highest = data.isNotEmpty ? data.reduce((a, b) => a['score'] > b['score'] ? a : b) : null;
    final lowest = data.isNotEmpty ? data.reduce((a, b) => a['score'] < b['score'] ? a : b) : null;
            ...
            Row(
              children: [
                ...
                Text(
                  "Weekly Mood Summary",
                  style: TextStyle(
                      color: Colors.brown[700], fontWeight: FontWeight.bold, fontSize: 19),
                ),
              ],
            ),
            const SizedBox(height: 20),
  ...
  Widget _summaryItem(String label, String value, IconData icon) => Column(
    children: [
      ...
      Text(value, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: Colors.brown[800])),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(color: Colors.brown[300], fontWeight: FontWeight.bold, fontSize: 12),
                  ),
                ],
              ),
            ),
